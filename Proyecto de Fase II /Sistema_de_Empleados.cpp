...
UsuarioTrabajador* registrarTrabajador(Empresa* empresa, map<string, UsuarioTrabajador*>& usuariosTrabajadores) {
    string nombre, dni, celular, afpTipo, id, pass;
    double sueldo;
    int hijos;

    cout << "Registro de trabajador\n";
    cout << "Nombre: "; cin >> nombre;
    do {
        cout << "DNI: "; cin >> dni;
            if (dni.length() !=8) {
                cout << "Error. DNI incorrecto, intente nuevamente\n";
            }
        }while(dni.length() !=8 );  
...
UsuarioAdministrador* registrarAdministrador(Empresa* empresa, map<string, UsuarioAdministrador*>& admins) {
    ...
    cout << "Nombre: "; cin >> nombre;
    do {
        cout << "DNI del empleado a actualizar: "; cin >> dni;
        if (dni.length() !=8) {
            cout << "Error. DNI incorrecto, intente nuevamente\n";
        }
    }while(dni.length() !=8 );  
    ...
    while (true) {
        ...
        if (op == 1) {
            ...
            if (trabajadores.count(id)) {
                ...
                t = registrarTrabajador(&empresa, trabajadores);
            }
                ...
                do {
                    ...
                    if (opcion == 1) {
                        ...
                        do {
                            ...
                            cin >> hijos;
                            if (hijos < 0) {
                                cout << "Error: cantidad inválida. Intente de nuevo.\n";
                            }
                        } while (hijos < 0);
                        ...
                        int faltas;
                        cout << "Ingrese numero de inasistencias: ";
                        cin >> faltas;
                        Boleta b(t, faltas);
                        b.mostrar();
                    }
