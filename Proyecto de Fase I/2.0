    ...
    void ReservarHabitacion(Habitacion* habitacion, string fechaInicio, string fechaFin, string codReserva) 
    {
        ...
        do {
            ...
            if (!esFechaValida(fechaInicio)) {
                cout << "(!) Formato de fecha inválido. Use dd/mm/aaaa con valores reales" << endl;
            }
        ...
        do {
            ...
            if (!esFechaValida(fechaFin)) {
                ...
                cout << "(!) La fecha de fin debe ser posterior a la de inicio" << endl;
                fechaFin = ""; // Forzar nueva entrada
            }
        } while (!esFechaValida(fechaFin) || fechaFin <= fechaInicio);
...
bool esFechaValida(const string& fecha) {
    ...
    // Verificar que todos los caracteres sean dígitos excepto las barras
    for (int i = 0; i < 10; i++) {
    ...
    // Extraer día, mes y año como strings
    string sDia = fecha.substr(0, 2);
    string sMes = fecha.substr(3, 2);
    string sAnio = fecha.substr(6, 4);

    // Convertir a números (similar a tu validación de DNI)
    int dia = stoi(sDia);
    int mes = stoi(sMes);
    int anio = stoi(sAnio);
    ...
    if (anio < 2023 || anio > 2100) return false; // Año entre 2023 y 2100
        ...
        if (opcionPrincipal == 1) {
            ...
            do {
                ...
                if (opcionUsuario == 1) {
                    ...
                    // Reservar con validación de fechas (ahora se piden dentro de la función)
                    usuarioActual->ReservarHabitacion(habitaciones[indice], "", "", codReserva);
                    
                    // Registrar reserva globalmente (opcional)
                    reservasGlobal.push_back(nullptr); // O almacenar la reserva real si es necesario
                }
